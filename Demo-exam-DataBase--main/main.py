...
class ClientLog(QDialog):
    ...
    def __init__(self,email,password):
        ...
        data = cur.execute(f"""SELECT Client.id, Client.first_name AS "Имя клиента", Client.second_name AS "Фамилия клиента", Client.last_name  AS "Отчество клиента", Uslugi.name_usl  AS "Название услуги", Worker.first_name AS "Имя учителя", Worker.second_name AS "Фамилия учителя",Worker.last_name AS "Отчество учителя", Main.date_usl AS "дата начала", Main.time_usl AS "Время начала"
FROM Main
...
class Workers(QDialog): 
    def __init__(self,email,password):
        ...
        self.conn = sqlite3.connect('LearnSchoolDB.db')
        cur = self.conn.cursor()
        data = cur.execute(f"""SELECT Main.id , Worker.first_name AS "Имя учителя", Worker.second_name AS "Фамилия учителя",Worker.last_name AS "Отчество учителя", Client.id AS "id клиента", Client.second_name AS "Имя клиента", Client.first_name AS "Фамилия клиента",  Client.last_name AS "Отчество клиента", Uslugi.name_usl AS "Название услуги",  Main.date_usl AS "дата начала", Main.time_usl AS "Время начала"
FROM Main
INNER JOIN Uslugi ON Main.id_usl = Uslugi.id_usl
INNER JOIN Worker ON Main.id_worker = Worker.id_worker
INNER JOIN Client ON Main.id_client = Client.id
