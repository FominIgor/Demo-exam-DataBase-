...
class Client(QDialog):
    ...
    def insert_staff(self):
        ...
        try:
            ...
            values('{self.leFio.text()}', '{self.leFio_2.text()}', '{self.leFio_3.text()}', '{'1' if self.rbMale.isChecked() else '2'}', '{self.sbAge.text()}', '{self.lePhone.text()}', '{self.leEmail.text()}', '{self.password11.text()}', '{self.sbAge_2.text()}')""")
            db.commit()
            self.conn.commit()
            cur.close()
        except Exception as e:
